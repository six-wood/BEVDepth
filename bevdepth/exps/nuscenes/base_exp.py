...
backbone_conf = {
    ...
    'z_bound': [-5, 3, 8],
    'd_bound': [2.0, 58.0, 0.5],  # 图像伪点云边界、网格定义
    'final_dim':
    final_dim,
    'output_channels':
    80,
...
bev_backbone = dict(  # BEV feature Encode参数
    type='ResNet',
    in_channels=80,
    depth=18,
    num_stages=3,
...
bev_neck = dict(type='SECONDFPN',  # 稀疏卷积neck
...
train_cfg = dict(  # 训练参数
    point_cloud_range=[-51.2, -51.2, -5, 51.2, 51.2, 3],  # 点云范围
    grid_size=[512, 512, 1],  # 网格大小
    voxel_size=[0.2, 0.2, 8],  # 网格尺寸，或许对性能有，LSS的网格尺寸为0.5
    out_size_factor=4,
    dense_reg=1,
...
class BEVDepthLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        mmcv.mkdir_or_exist(default_root_dir)  # 输出？
        self.default_root_dir = default_root_dir
        self.evaluator = DetNuscEvaluator(class_names=self.class_names,
                                          output_dir=self.default_root_dir)  # 评价指标
    ...
    def training_step(self, batch):  # 一个batch中train的行为
    ...
    def get_depth_loss(self, depth_labels, depth_preds):  # 深度监督损失
    ...
    def get_downsampled_gt_depth(self, gt_depths):  # 降采样深度图
        """
        Input:
            gt_depths: [B, N, H, W]
        Output:
    ...
    def eval_step(self, batch, batch_idx, prefix: str):  # 一个batch中eval的行为
    ...
    def validation_epoch_end(self, validation_step_outputs):  # 多个node训练，在一个epoch结尾调用
    ...
    def test_epoch_end(self, test_step_outputs):  # 多个node训练，在一个epoch结尾调用
        all_pred_results = list()
        all_img_metas = list()
    ...
    def train_dataloader(self):
        train_dataset = NuscDetDataset(ida_aug_conf=self.ida_aug_conf,
                                       ...
                                       use_fusion=self.use_fusion)  # 定义NuscDetDataset
