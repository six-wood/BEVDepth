...
class _ASPPModule(nn.Module):  # 空洞卷积块
    ...
    def _init_weight(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                torch.nn.init.kaiming_normal_(m.weight)  # 卷积初始化
            elif isinstance(m, nn.BatchNorm2d):
                m.weight.data.fill_(1)  # batchNorm初始化
                m.bias.data.zero_()


class ASPP(nn.Module):  # 空洞卷积金字塔，结构与论文一致
...
class Mlp(nn.Module):  # 经典MLP（无隐藏层）
...
class SELayer(nn.Module):  # channel Attention

    def __init__(self, channels, act_layer=nn.ReLU, gate_layer=nn.Sigmoid):
        super().__init__()
        self.conv_reduce = nn.Conv2d(channels, channels, 1, bias=True)  # 聚合
        self.act1 = act_layer()
        self.conv_expand = nn.Conv2d(channels, channels, 1, bias=True)  # 预测重要性
...
class DepthNet(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, context_channels,
                 depth_channels):  # in、mid、context+depth
        ...
        self.depth_conv = nn.Sequential(
            BasicBlock(mid_channels, mid_channels),  # resnet basic block
            BasicBlock(mid_channels, mid_channels),
            BasicBlock(mid_channels, mid_channels),
            ASPP(mid_channels, mid_channels),  # 多尺度空洞卷积
...
class BaseLSSFPN(nn.Module):
    ...
    def get_geometry(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat):  # 伪点云坐标系变换
    ...
    def get_cam_feats(self, imgs):  # 获取图像特征
